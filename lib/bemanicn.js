...
function buildGeocodeAddress(shop, countyName) {
...
function normalizeShop(shop, countyMap) {
  ...
  return {
    ...
    geocodeAddress: buildGeocodeAddress(shop, countyName),
...
function parseBemanicnCityHtml(html) {
  ...
  const shops = city.shops
    .map((shop) => normalizeShop(shop, countyMap))
    .sort((a, b) => a.id - b.id);
  ...
  return {
    ...
    source: {
      name: 'BEMANICN 街机地图',
      url: 'https://map.bemanicn.com/region/city/110100000000',
